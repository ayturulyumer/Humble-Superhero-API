export default function SuperheroList({ superheroes }) {
    return (
        <div className="mt-8">
            <h2 className="text-2xl font-bold mb-4 text-gray-800">Superhero List</h2>
            {superheroes.length === 0 ? (
                <p className="text-gray-500">No superheroes added yet.</p>
            ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {superheroes.map((hero, index) => (
                        <div key={index} className="bg-gradient-to-r from-purple-100 to-pink-100 p-4 rounded-lg shadow-md flex flex-col justify-between">
                            <div>
